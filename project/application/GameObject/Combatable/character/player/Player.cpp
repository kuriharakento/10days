...
#pragma region // 参考
	// 試しに腕を追加
	auto Rarm = std::make_unique<CombatableObject>(GameObjectTag::Weapon::PlayerBullet);
	Rarm->Initialize(object3dCommon, lightManager);
	Rarm->SetModel("cube");
	Rarm->SetPosition(Vector3(3.0f, 0.0f, 0.0f));
	Rarm->SetAttackPower(100.0f);
	...
	AddChild(GameObjectTag::Weapon::PlayerBullet, std::move(Rarm));
